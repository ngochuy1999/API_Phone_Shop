...
public class AccountService {
    ...
    public Integer loginUser(String email, String password) {
        ...
        Customer userExists = accountDAO.findByEmail(email);
        Account accountEx = userExists.getAccount();

        if (!password.equals(accountEx.getPassword())||accountEx.getRole().getRoleId() != 3 ) {
            return 0;
        } else {
            if(accountEx.getIsAccuracy() == 1) return userExists.getUserId();
            else return -1;
        }
    }
...
//        }
    ...
    public boolean changePassword(int id,String oldPassword, String newPassword){
        return accountDAO.changePassword(id,oldPassword,newPassword);
    }
